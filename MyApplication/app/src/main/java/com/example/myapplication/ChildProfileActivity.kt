...
class ChildProfileActivity : AppCompatActivity() {
    ...
    private fun loadFullStats() {
        api.getChildFullStats("Token $token", childId).enqueue(object : Callback<ChildFullStatsResponse> {
            override fun onResponse(call: Call<ChildFullStatsResponse>, response: Response<ChildFullStatsResponse>) {
                if (response.isSuccessful && response.body() != null) {
                    val stats = response.body()!!

                    // Общая статистика уже есть в tvBooksCount и tvPagesCount
                    ...
                    binding.tvAverageScore.text = "${stats.avg_score_percent.toInt()}%"

                    // Дополнительная статистика
                    binding.tvTotalReadingTime.text = stats.total_reading_time_formatted
                    binding.tvTotalWords.text = stats.total_words.toString()
                    binding.tvReadingSpeed.text = "${stats.reading_speed_wpm.toInt()} слов/мин"
                }
            }
            override fun onFailure(call: Call<ChildFullStatsResponse>, t: Throwable) {
    ...
    private fun canDeleteBook(book: Book): Boolean {
        if (isParentViewing) return true
        if (book.uploaded_by_id != null && book.uploaded_by_id != childId) return false
        return true
    }
    ...
    private fun deleteBook(book: Book) {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Удалить") { _, _ -> performDelete(book) }
    ...
    inner class BooksAdapter(
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            holder.binding.btnEditGoal.setOnClickListener { editDailyGoal(book) }
            holder.binding.btnDelete.setOnClickListener { deleteBook(book) }

            holder.binding.root.setOnClickListener {
                val intent = Intent(this@ChildProfileActivity, ChildBookStatsActivity::class.java)
                intent.putExtra("child_id", childId)
                intent.putExtra("child_name", childName)
